...
import { LayoutDashboard, ClipboardList, Users, Wrench, LogOut, Search } from "lucide-react";
...
export function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-50 flex">
      {/* Sidebar */}
      <aside className="w-64 bg-white border-r flex flex-col hidden md:flex h-[100dvh] sticky top-0">
        <div className="p-6 border-b">
          <h1 className="font-bold text-xl text-primary">HomeCare Admin</h1>
          {user && <p className="text-sm text-gray-500 mt-1">{user.name}님 환영합니다</p>}
        </div>
        ...
        <div className="p-4 border-t">
          <Button variant="ghost" className="w-full justify-start text-gray-500 hover:text-gray-900" onClick={logout} data-testid="btn-logout">
